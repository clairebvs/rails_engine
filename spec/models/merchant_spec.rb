...
RSpec.describe Merchant, type: :model do
  ...
  describe '.total_revenue_all_merchants_on_date' do
    it 'should return the total revenue for all merchants on a date' do
      merchant_id = create(:merchant).id
      customer_id = create(:customer).id
      invoice = create(:invoice, merchant_id: merchant_id, customer_id: customer_id, updated_at: '2018-03-09')
      item_id = create(:item, merchant_id: merchant_id).id
      invoice_items_1 = create(:invoice_item, invoice_id: invoice.id, item_id: item_id, quantity: 2, unit_price: 4)

      date = '2018-03-09'
      parsed_date = Date.parse("#{date} 00:00:00 UTC")
      expect(Merchant.total_revenue_all_merchants_on_date(parsed_date)).to eq(0)
    end
  end
